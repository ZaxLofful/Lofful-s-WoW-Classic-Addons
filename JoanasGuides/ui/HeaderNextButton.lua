...
function component.Init(components)
	...
	frame:SetSize(20,20)
	frame:SetPoint("RIGHT", parent, "RIGHT", -6, 4)
	frame:Disable()
	AddNormalTexture(frame, I["UI-SquareButton-Right-Up"])
	AddDisabledTexture(frame, I["UI-SquareButton-Right-Disabled"])
	AddHighlightTexture(
			frame,
			CreateTexture("Interface\\Buttons\\UI-Common-MouseHilight",nil,nil, "ADD"))
	AddPushedTexture(frame, I["UI-SquareButton-Right-Down"])
...
function component.Update()
	if (component:IsDirty()) then
		if (GuideNavigationService.IsGuideSet()) then
			component.frame:SetEnabled(GuideNavigationService.HasNextStep())
